...
以下内容一些来自论文原文或翻译, 一些来自互联网上的其他文章,
还有一些为自己的理解, 如有错误还望能够在评论中指出.

## B-树 (B-Tree)

`B-Tree`, 1970 年由 Bayer,R 与 McCreight,E 于 ["ORGANIZATION AND MAINTENANCE OF LARG"][paper-btree] 中首次提出.
...
这样我们便将问题向上一层进行了转移 (原本 `x2/x6` 层的问题转变到了 `P` 所在层),
进行递归修复直到满足性质或者到达 ROOT 节点, 此时生成新的 ROOT 节点 `x4`.
